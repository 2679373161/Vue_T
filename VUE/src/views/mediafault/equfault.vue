...
<el-table  ref="tableRef" :data="tableData" :cell-style="cellStyle"  height="900px" border style="width: 100%;margin-top:20px" align="center">
  ...
  <el-table-column  prop="prod_time" align="center" width='150px' label="生产日期"></el-table-column>
...
export default {
  ...
  methods:{
   ...
   // cellStyle(row,column,rowIndex,columnIndex){
      ...
      //  if(row.row.e0>0||row.row.e1>0||row.row.e2>0||row.row.e3>0||row.row.e4>0||row.row.e5>0||row.row.e6>0||row.row.e8>0||row.row.ea>0||row.row.ee>0||row.row.f2>0||row.row.c0>0||row.row.c1>0||row.row.c2>0||row.row.c3>0||row.row.c4>0||row.row.c5>0||row.row.c6>0||row.row.c7>0||row.row.c8>0||row.row.eh>0||row.row.ef>0){
...
console.log(this.temp_currentPage)
    request.post('faultstatistics/page/idfault', null, {
      params:
           { 
...
    return year+"-"+month+"-"+day;
    ...
    handleSelect() {
      ...
      if (this.value2.length == 0) {//型号空时
        ...
        request.post('faultstatistics/page/getequ', null, {
          params:
          { 
...
this.value1[0]=new Date(this.$route.query.prodmonth)
this.value1[1]=new Date(this.$route.query.prodmonth)
 //this.value1[0]=this.$route.query.prodmonth
 //this.value1[1]=this.$route.query.prodmonth
