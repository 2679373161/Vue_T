...
<el-table  ref="tableRef" :data="tableData" height="900px" border style="width: 100%;margin-top:20px" align="center">
   ...
   <el-table-column   prop="overshoot_num" align="center" width='150px' label="超调异常片段数"></el-table-column>
 <el-table-column  prop="osp" align="center" width='120px' label="超调异常占比"></el-table-column>
       <el-table-column  prop="avg_overshoot" align="center" width='150px' label="平均超调量"></el-table-column> 
...
if(this.fanye==0){
  this.temp_currentPage=1
    ...
    request.post('fault2/page/faultdiagnosis2', null, {
      params:
           { 
          flag: 3,
 ...
 if (response.data.data==""){
        ...
        for(var i=0;i<response.data.data.length;i++){
          ...
          time_date:response.data.data[i].start_time,
          current_level_num:response.data.data[i].current_level_num,

          total_num:response.data.data[i].total_num,
    
          overshoot_num:response.data.data[i].overshoot_num,
          osp:response.data.data[i].osp,
          avg_overshoot:response.data.data[i].avg_overshoot,
          ...
          avg_temp_diff:response.data.data[i].avg_temp_diff,
            ...
            this.tableData[i].fault_type="超调异常"
...
this.shijian=this.$route.query.time//接收到的日期时间，需转换为时间戳再减去13天的时间戳，再转换为日期
 ...
 if(this.$route.query.id!=null){
  ...
  const sj=new Date(this.datatime[0])
   sj.setTime(sj.getTime()-86400000*30);  //一周
  console.log(sj)
this.datatime[0]=this.dateFormat(sj)//一周前的时间
  ...
  dt1.setTime(dt.getTime()-86400000*30);  //一周
