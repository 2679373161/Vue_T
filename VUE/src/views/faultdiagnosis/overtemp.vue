...
  this.temp_currentPage=1
      ...
      this.tableData=[]
      this.total=0
    request.post('fault/page/overtemp', null, {
      params:
        { 
      ...
      console.log(response.data)
        ...
        for(i=0;i<this.tableData.length;i++){
          this.tableData[i].time_date=response.data.data[i].start_time+"-"+response.data.data[i].time_date
        }
...
      this.fanye=0
    ...
    handleChange() {
       this.value2 = []
       this.value3 = []
       request.post('fault/page/overtempgettype', null, {
        params:
          { 
    ...
    handleSelect() {
      ...
      if (this.value2.length == 0) {//型号空时
        ...
        request.post('fault/page/overtempgetid', null, {
          params:
          { 
            flag: shebeiflag,
            province_code: this.value[0],
  ...
  created() {
     request.post('fault/page/overtempgetdata', null, {
         params:
         { 
         },
       }).then(({ data: response }) => {
