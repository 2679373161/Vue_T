<template>
  ...
  <el-form ref="form" class="ItemInfo" :model="equipment_info" label-width="80px" :inline="true">
    ...
    <span style="margin-left:30px;">型号选取：</span> 
    <el-cascader
      v-model="value"
      ref="cascaderAddr"
      :options="options"
 ...
 <download-excel
    ...
    <div class="content" style="margin-top:20px">
      <h4 align="left"><strong>{{model[0]}}至{{model[1]}}月型号信息汇总</strong></h4>
      <el-tabs v-model="activeName" type="card">
        <el-tab-pane label="表格" name="first">
          <el-table :data="tableData" ref="filterTable" @sort-change="changeTableSort" border style="width: 100%;margin-top:10px" align="center" :cell-style="cellStyle" row-key="id" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" :default-sort = "{prop: 'temp_score', order: 'ascending'}">
            ...
            <el-table-column  prop="ave_heat_duration" align="center" sortable label="加热时长/台"></el-table-column>
            <el-table-column  prop="ave_un_sable_duration" align="center" sortable  label="不恒温时长/台"></el-table-column>
...
var lengend1={'温度评估分数':true,'恒温异常模式占比':false,'不恒温时长占比':false,'加热时长/台':false,'不恒温时长/台':false,'加热+恒温异常模式占比':false,'温度有效片段':false}
...
startDownload(){
    ...
    getOptionlist(){
      for(var i=0;i<this.tableData.length;i++){
        ...
        this.headerData[0].option[i]={label:this.tableData[i].dev_type,value:this.tableData[i].dev_type}
    ...
    filterprovinces(){
      ...
      for(var i=0;i<this.headerData[0].value.length;i++){
        ...
        for(var j=0;j<this.tableData1.length;j++){
          if(this.tableData1[j].dev_type==this.headerData[0].value[i]){
    ...
    selectallprovince(){
      ...
      for(var j=0;j<this.tableData1.length;j++){
        this.headerData[0].value[j]=this.tableData1[j].dev_type
    ...
    cellStyle(row,column,rowIndex,columnIndex){
      ...
      if(row.row.time_date==this.model[0]+'~'+this.model[1]){
        if(row.row.abnormal_stable_temp_pro>50){
    ...
    changeTrendorder(){
      ...
      if(this.radio==='2'){
        var space=0;
        for(var i=0;i<abnormal_heat_pro.length-1;i++){
          for(var j=i+1;j<abnormal_heat_pro.length;j++){
            if(conditon[i]>conditon[j]){
              space=this.abnormal_heat_pro[i];
              this.abnormal_heat_pro[i]=this.abnormal_heat_pro[j];
              this.abnormal_heat_pro[j]=space;
              ...
              space=this.temp_num[i];
              this.temp_num[i]=this.temp_num[j];
              this.temp_num[j]=space;
    ...
    //表格添加子数组，并计算均值数据
    tableAddTreeprops(data,dev_typeNotrepetitive){
      console.log(data,dev_typeNotrepetitive)
      var averageData=[];
      var equipment_num=0;
      ...
      template=[];
      var data1=new Array();
      var dev_typeNotrepetitive1=new Array();
      for(var i=0;i<data.length;i++){
        data1[i]=data[i];
      }
      for(var j=0;j<dev_typeNotrepetitive.length;j++){
        dev_typeNotrepetitive1[j]=dev_typeNotrepetitive[j]
        template[j]=new Array();
        for(var i=0;i<data1.length;i++){
          if(data1[i].dev_type==dev_typeNotrepetitive1[j]){
            ...
            totalTime+=(data1[i].abnormal_all_pro);
            temp_num+=data1[i].temp_num;
            ave_unstable_proportion+=data1[i].ave_unstable_proportion;
        ...
        averageData[j]={id: j+1+'a',dev_type: dev_typeNotrepetitive[j],time_date: this.model[0]+'~'+this.model[1],equipment_num: Math.floor(equipment_num/k),abnormal_heat_pro: Number(parseFloat(abnormal_heat_pro/k).toFixed(2)),
                        abnormal_stable_temp_pro: Number(parseFloat(abnormal_stable_temp_pro/k).toFixed(2)),abnormal_all_pro: Number(parseFloat(totalTime/k).toFixed(2)),temp_num: Math.floor(temp_num/k),
                        ave_unstable_proportion: Number(parseFloat(ave_unstable_proportion/k).toFixed(2)),ave_heat_duration: this.sec2time(Math.floor(ave_heat_duration/k)),ave_un_sable_duration: this.sec2time(ave_un_sable_duration),minimum_time: this.sec2time(minimum_time),
    ...
    search(){
      ...
      request.post('tableStoreDates/page/list', null, {
        ...
        this.tableData=this.tableAddTreeprops(response.data.data,response.data.dev_type);
        ...
        for(var i=0;i<this.tableData.length;i++){
          totalTime[i]=this.tableData[i].abnormal_all_pro;//加热+恒温异常
          abnormal_stable_temp_pro[i]=this.tableData[i].abnormal_stable_temp_pro;//恒温异常模式占比
          ave_heat_duration[i]=this.time2sec(this.tableData[i].ave_heat_duration);//加热时长/台
          abnormal_heat_pro[i]=this.time2sec(this.tableData[i].ave_un_sable_duration);
          totalNum[i]=this.tableData[i].ave_unstable_proportion;
          temp_num[i]=this.tableData[i].temp_num;
          province[i]=this.tableData[i].dev_type
         temp_score[i]=this.tableData[i].temp_score
        }
        // province=response.data.dev_type;
        ...
        this.abnormal_heat_pro=abnormal_heat_pro;
  ...
  created() {
    ...
    request.post('tableStoreDates/page/list', null, {
        ...
        for(var i=0;i<response.data.data.length;i++){
          ...
          allOption[i][0]=response.data.data[i].dev_type;
        }
