<template>
  <div class="page"  style="margin-top:20px;">
    ...
    <el-table ref="tableRef" :data="Equment_AllInfo.slice((currentPage-1)*perPage,currentPage*perPage)" :cell-style="cellStyle"  max-height="900" border style="width: 100%;margin-top:0.5rem" align="center">
...
export default {
 ...
 query: {
   ...
   type: row.type,
   time0:this.Equment_info.TimeData[0],
   time1:this.Equment_info.TimeData[1]
  }
});
 window.open(routeData.href);
    ...
    AllInfo_init() {
      ...
      request.post('faultstatistics/page/typesearchsum', null, {
        params:
           {
          ...
          for (var listnum = 0; listnum < this.ErrorList.length; listnum++) { 
            if ((this.ErrorList[listnum].value.toLowerCase()) == (response.data.MideaTypeFault_info[index].max_error).toLowerCase()) {
              this.Equment_AllInfo[index] = {
                time: this.Equment_info.TimeData[0]+"-"+this.Equment_info.TimeData[1],
