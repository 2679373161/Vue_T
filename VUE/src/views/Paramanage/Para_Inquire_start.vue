    ...
    deal_timer_para() { // 等待参数写入完成
      ...
      if(this.para_table_data == true) {
        ...
        this.button_state = false;
          ...
          if(this.activeName_temp == "second" || this.activeName_temp == "fifth"){//总参数下发才关机
            this.straupdiv();  
          }
...
if(Index_num==4){//如果是总参数查询就要先关机
  request.post('sentcmd/downdiv', null, {//设备读取参数之前的关机
            params:
            {
            applianceid: this.equipment_ID,
            },
             ...
             this.button_content = '设备关机中';
             this.button_state = true;
	setTimeout(() => {
      
             this.Deal_para(Index_num)
     }, 3000);//延时3s请求
...
                        this.timer_flag_para_getinfo = null
    ...
    this.Deal_para(Index_num)//不是查总参数就无需关机，直接查
}
...
straupdiv(){
 request.post('sentcmd/straupdiv', null, {//设备开机
            params:
            {
            applianceid: this.equipment_ID,
            },
...
                        this.timer_flag_para_getinfo = null
    ...
    outdowndivdefault(){//默认下发参数关机


 if(this.activeName_temp == "second" || this.activeName_temp == "fifth"){
     this.button_txt_para_default = '设备关机中...'
    request.post('sentcmd/downdiv', null, {//设备读取参数之前的关机
            params:
            {
            applianceid: this.equipment_ID,
...
                        this.timer_flag_para_getinfo = null
    ...
    outdowndiv(){
         ...
         if(this.activeName_temp == "second" || this.activeName_temp == "fifth"){
             this.button_txt_para = '设备关机中...'
    request.post('sentcmd/downdiv', null, {//设备下发参数之前的关机
            params:
            {
            applianceid: this.equipment_ID,
            },
