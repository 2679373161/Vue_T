...
function out_default(table, name, callback) {  // table为表格数据,name为导出文件名,
  // callback为导出完毕回调,方便你知道导出完成了(可根据自己需求决定是否需要)
  let tableInnerHTML = ''
  let headerData = ['applianceid']
  ...
  tableInnerHTML += `<th colspan=${headerData.length} style='background:#CCFFFF;border:solid;'>` + "多设备修改状态信息" + "</th></tr>"
  tableInnerHTML += '<tr>' 
  ...
  bodyData.forEach(item => {
    tableInnerHTML += "<tr>"
    tableInnerHTML += `<td STYLE='MSO-NUMBER-FORMAT:\\@'"align='center' style='border:solid'>${item.equipment_ID}</td>`
    // tableInnerHTML += "<td align='center' style='border:solid'>" + item.equipment_State + "</td>"
    // tableInnerHTML += "<td align='center' style='border:solid'>" + item.equipment_Time + "</td>"
    // tableInnerHTML += "<td align='center' style='border:solid'>" + item.Isrewirted + "</td>"
...
let tableToExcel = (function () {
  ...
  return function (table, name, callback) {
    ...
    a.remove();
    callback('success');    
    /* 这里调用我们自己传入的回调方法，这样导出Excel完成后你就能在外面知道导出完毕，并且再往下做自己其他的逻辑 */
  };
}());
export default out_default;    // 导出自己封装的transform方法
