...
　　}
    ...
    //     filters[i]={text:this.tableData[i].dev_type,value:this.tableData[i].dev_type}
    //   }
    //   console.log(filters)
    //   return filters;
    // },
    ...
    tableAddTreeprops(data,dev_typeNotrepetitive){
      ...
      for(var j=0;j<dev_typeNotrepetitive.length;j++){
        ...
        averageData[j]={id: j+1+'a',dev_type: dev_typeNotrepetitive[j],time_date: this.model[0]+'~'+this.model[1],equipment_num: Math.floor(equipment_num/k),un_stable_proportion: Number(parseFloat(un_stable_proportion/k).toFixed(2)),
