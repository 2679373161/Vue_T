<template>
  ...
  <div>
    ...
    <div class="search1" align="center" v-if="flag==true">
...
<el-form-item class="ItemInfoTime" style="margin-left:10px;">
    ...
    <div class="search2" align="center" v-if="flag1==true">   
      <el-form ref="form" class="ItemInfo" :model="equipment_info" label-width="80px" :inline="true">
 <!--型号.搜索部分-->
    <span>型号选取：</span> 
    <el-cascader
    ...
    <div class="content2" style="margin-top:20px">                       <!--菜单下所有标签、图表部分-->
      ...
      <el-tabs v-model="activeName" type="card">
        <el-tab-pane label="表格" name="first"><!--表格切换防止出现渲染问题（二选一）：1.给不同表格添加各自的key；2.直接使用v-show，不用v-if-->
          <el-table :data="tableData" v-if="flag==true" key="one" border style="width: 100%;margin-top:10px" align="center" :cell-style="cellStyle" >
          ...
          <el-table :data="tableData2" v-if="flag2==true" key="three" border style="width: 100%;margin-top:10px" align="center" :cell-style="cellStyle" >
  ...
  startDownload(){
    ...
    initialLocation(){
      ...
      console.log(1)
        ...
        this.value1=[response.data.lastmonth,response.data.lastmonth]
      }).catch((err) => {
      });
        console.log(1)
    ...
    initialType(){
      ...
      request.post('tableStoreDates/page/list', null, {
        ...
        console.log(response.data.lastmonth)
        this.value3=[response.data.lastmonth,response.data.lastmonth]
      }).catch((err) => {
      });
        // this.value3=['2021-01','2021-12'];
        this.value7='1';
    ...
    initialAppliance(){
      ...
      request.post('tableStoreDates/page/list', null, {
        ...
        console.log(response.data.lastmonth)
        this.value6=[response.data.lastmonth,response.data.lastmonth]
      }).catch((err) => {
      });
